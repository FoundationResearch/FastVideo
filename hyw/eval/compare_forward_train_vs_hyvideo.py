...
def main() -> None:
    ...
    x0 = latent_pt["latents"].to(device=device, dtype=torch.bfloat16)  # (1,C,T,H,W)
    B, _, T, _, _ = x0.shape

    # Build conditioning tensors
    prompt_embeds = latent_pt["prompt_embeds"].to(device=device, dtype=torch.bfloat16)
    prompt_mask = latent_pt["prompt_mask"].to(device=device)
    image_cond = latent_pt["image_cond"].to(device=device, dtype=torch.bfloat16)  # (1,C,1,H,W)
    vision_states = latent_pt["vision_states"].to(device=device, dtype=torch.bfloat16)
    byt5_text_states = latent_pt["byt5_text_states"].to(device=device, dtype=torch.bfloat16)
    ...
    viewmats = viewmats.unsqueeze(0).to(device=device, dtype=torch.bfloat16)  # (1,T,4,4)
    Ks = Ks.unsqueeze(0).to(device=device, dtype=torch.bfloat16)
    action_vec = action_labels.reshape(-1).to(device=device, dtype=torch.bfloat16)  # (T,)
    ...
    eps = torch.randn(x0.shape, device=device, dtype=torch.float32).to(dtype=x0.dtype)
    sigma = float(args.sigma)
    x_t = (1.0 - sigma) * x0 + sigma * eps
    cond_latents = image_cond.repeat(1, 1, x0.shape[2], 1, 1)
    hidden_states = torch.cat([x_t, cond_latents], dim=1)

    timestep = torch.full((B * T,), float(args.t_value), device=device, dtype=torch.bfloat16)
    timestep_txt = torch.tensor(0, device=device, dtype=torch.bfloat16).unsqueeze(0)
    ...
    hyvideo_model = HyvideoTransformer.from_pretrained(base_transformer_dir, torch_dtype=torch.bfloat16, low_cpu_mem_usage=True)
    ...
    trainer_model = trainer_model.to(device).eval()
    hyvideo_model = hyvideo_model.to(device).eval()
