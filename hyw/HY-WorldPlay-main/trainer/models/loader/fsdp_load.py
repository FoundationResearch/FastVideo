...
# Copyright 2025 The Trainer Authors.
...
from trainer.models.loader.weight_utils import safetensors_weights_iterator
...
def build_2d_groups(rank: int, world_size: int, fsdp_group_size: int):
    assert world_size % fsdp_group_size == 0, "world_size 必须能整除 FSDP_GROUP_SIZE"
    num_replicas = world_size // fsdp_group_size

    shard_group_id = rank // fsdp_group_size
    shard_index    = rank % fsdp_group_size
    shard_ranks = list(range(shard_group_id * fsdp_group_size,
                             (shard_group_id + 1) * fsdp_group_size))
    fsdp_pg = dist.new_group(ranks=shard_ranks)

    replica_ranks = [shard_index + k * fsdp_group_size for k in range(num_replicas)]
    ddp_pg = dist.new_group(ranks=replica_ranks)

    ddp_rank_in_group = replica_ranks.index(rank)

    info = {
        "fsdp_pg": fsdp_pg,
        "ddp_pg": ddp_pg,
        "num_replicas": num_replicas,
        "ddp_rank": ddp_rank_in_group,
...
def fsdp_wrap(module, sharding_strategy="full", mixed_precision=False, wrap_strategy="size", process_group=None, min_num_params=int(5e7), transformer_module=None, cpu_offload=False):
    if mixed_precision:
