...
class LTX2DistillationPipeline(DistillationPipeline):
    ...
    def initialize_training_pipeline(self, training_args: TrainingArgs):
        ...
        # Backward-compatible fallback to shared guidance when per-modality
        # scales are not provided.
        self.real_score_guidance_scale_video = (
            self.training_args.real_score_guidance_scale_video
            if self.training_args.real_score_guidance_scale_video is not None
            else self.real_score_guidance_scale)
        self.real_score_guidance_scale_audio = (
            self.training_args.real_score_guidance_scale_audio
            if self.training_args.real_score_guidance_scale_audio is not None
            else self.real_score_guidance_scale)
        self.real_score_stg_scale_video = (
            self.training_args.real_score_stg_scale_video)
        self.real_score_stg_scale_audio = (
            self.training_args.real_score_stg_scale_audio)
        self.real_score_modality_scale_video = (
            self.training_args.real_score_modality_scale_video)
        self.real_score_modality_scale_audio = (
            self.training_args.real_score_modality_scale_audio)
        self.real_score_rescale_scale = (
            self.training_args.real_score_rescale_scale)
    ...
    def _prepare_validation_batch(self, sampling_param, training_args,
        ...
        # Keep LTX-2 per-modality CFG aligned with guidance_scale unless
        # explicitly overridden by the validation sample.
        if validation_batch.get("ltx2_cfg_scale_video") is not None:
            sampling_param.ltx2_cfg_scale_video = float(
                validation_batch["ltx2_cfg_scale_video"])
        else:
            sampling_param.ltx2_cfg_scale_video = float(
                sampling_param.guidance_scale)
        if validation_batch.get("ltx2_cfg_scale_audio") is not None:
            sampling_param.ltx2_cfg_scale_audio = float(
                validation_batch["ltx2_cfg_scale_audio"])
        else:
            sampling_param.ltx2_cfg_scale_audio = float(
                sampling_param.guidance_scale)
        if validation_batch.get("ltx2_stg_scale_video") is not None:
            sampling_param.ltx2_stg_scale_video = float(
                validation_batch["ltx2_stg_scale_video"])
        if validation_batch.get("ltx2_stg_scale_audio") is not None:
            sampling_param.ltx2_stg_scale_audio = float(
                validation_batch["ltx2_stg_scale_audio"])
        if validation_batch.get("ltx2_modality_scale_video") is not None:
            sampling_param.ltx2_modality_scale_video = float(
                validation_batch["ltx2_modality_scale_video"])
        if validation_batch.get("ltx2_modality_scale_audio") is not None:
            sampling_param.ltx2_modality_scale_audio = float(
                validation_batch["ltx2_modality_scale_audio"])
        if validation_batch.get("ltx2_rescale_scale") is not None:
            sampling_param.ltx2_rescale_scale = float(
                validation_batch["ltx2_rescale_scale"])
    ...
    def _dmd_forward(self, generator_pred_video, training_batch):
        ...
        with torch.no_grad():
            ...
            # STG perturbed teacher prediction (skip self-attention).
            training_batch = self._build_distill_input_kwargs(
                noisy_latent,
                timestep,
                training_batch.conditional_dict,
            ...
            real_stg = current_real_score_transformer(
                **training_batch.input_kwargs)
            if isinstance(real_stg, tuple):
                real_stg_video, real_stg_audio = real_stg
            else:
                real_stg_video, real_stg_audio = real_stg, None

            # Modality-isolated teacher prediction (cross-modal attention off).
            training_batch = self._build_distill_input_kwargs(
                noisy_latent,
                timestep,
                training_batch.conditional_dict,
            ...
            real_modality = current_real_score_transformer(
                **training_batch.input_kwargs)
            if isinstance(real_modality, tuple):
                real_modality_video, real_modality_audio = real_modality
            else:
                real_modality_video, real_modality_audio = real_modality, None
            ...
            if training_batch.unconditional_dict is not None:
                ...
                ) * (self.real_score_guidance_scale_video - 1)
                real_score_pred_video = real_score_pred_video + (
                    real_cond_video - real_stg_video
                ) * self.real_score_stg_scale_video
                real_score_pred_video = real_score_pred_video + (
                    real_cond_video - real_modality_video
                ) * (self.real_score_modality_scale_video - 1)
                if real_cond_audio is not None and real_uncond_audio is not None:
                    ...
                    ) * (self.real_score_guidance_scale_audio - 1)
                    if real_stg_audio is not None:
                        real_score_pred_audio = real_score_pred_audio + (
                            real_cond_audio - real_stg_audio
                        ) * self.real_score_stg_scale_audio
                    if real_modality_audio is not None:
                        real_score_pred_audio = real_score_pred_audio + (
                            real_cond_audio - real_modality_audio
                        ) * (self.real_score_modality_scale_audio - 1)
                else:
                    real_score_pred_audio = real_cond_audio

                # Guidance rescaling (prevents saturation), matching inference.
                if self.real_score_rescale_scale > 0:
                    f_v = real_cond_video.std() / real_score_pred_video.std()
                    f_v = (self.real_score_rescale_scale * f_v +
                           (1 - self.real_score_rescale_scale))
                    real_score_pred_video = real_score_pred_video * f_v
                    if (real_cond_audio is not None
                            and real_score_pred_audio is not None):
                        f_a = real_cond_audio.std() / real_score_pred_audio.std()
                        f_a = (self.real_score_rescale_scale * f_a +
                               (1 - self.real_score_rescale_scale))
                        real_score_pred_audio = real_score_pred_audio * f_a
            else:
