...
class WanActionSelfAttention(nn.Module):
    ...
    def forward(self,
        ...
        Unified code path for both training and inference (matching original HY-WorldPlay).
        ...
        # Apply RoPE manually (matching original - unified path for train/inference)
        cos, sin = freqs_cis
        ...
        # KV cache handling (unified path matching original HY-WorldPlay)
        ...
        # DEBUG: Save all tensors to single file
        if os.environ.get("DEBUG_SAVE_TENSORS", "0") == "1" and hasattr(self, '_debug_block_idx') and self._debug_block_idx == 0:
            debug_tensors = {
                "query": q.transpose(1, 2).cpu(),
                "key": k.transpose(1, 2).cpu(),
                "value": v.transpose(1, 2).cpu(),
                "query_rope": query_rope.transpose(1, 2).cpu(),
                "key_rope": key_rope.transpose(1, 2).cpu(),
                "query_prope": query_prope.transpose(1, 2).cpu(),
                "key_prope": key_prope.transpose(1, 2).cpu(),
                "value_prope": value_prope.transpose(1, 2).cpu(),
                "query_all": query_all.transpose(1, 2).cpu(),
                "key_all": key_all.transpose(1, 2).cpu(),
                "value_all": value_all.transpose(1, 2).cpu(),
                "attn_output": hidden_states_all.transpose(1, 2).cpu(),
            }
            torch.save(debug_tensors, "debug_fv.pt")
            print(f"[FV] Saved debug tensors to debug_fv.pt")
...
class WanActionTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        # Cast temb to float32 for scale/shift computation (matching original HY-WorldPlay)
        ...
        # DEBUG: Log after norm1 and set debug index on attention
        if hasattr(self, '_block_idx'):
            self.attn1._debug_block_idx = self._block_idx
        if os.environ.get("DEBUG_TRANSFORMER", "0") == "1" and hasattr(self, '_block_idx') and self._block_idx == 0:
            print(f"[FV Block {self._block_idx}] norm1_out: mean={norm_hidden_states.float().mean().item():.6f}, std={norm_hidden_states.float().std().item():.6f}, shape={norm_hidden_states.shape}")
        ...
        # Self-attention residual + norm in float32 (matching original HY-WorldPlay)
        ...
        # DEBUG: Log after self-attention
        if os.environ.get("DEBUG_TRANSFORMER", "0") == "1" and hasattr(self, '_block_idx') and self._block_idx == 0:
            print(f"[FV Block {self._block_idx}] self_attn_out: mean={hidden_states.float().mean().item():.6f}, std={hidden_states.float().std().item():.6f}")
        ...
        # Cross-attention residual in bfloat16 (matching original: hidden_states + attn_output)
        hidden_states = hidden_states + attn_output
        
        # norm3 for FFN input in float32 (matching original HY-WorldPlay)
        norm_hidden_states = self.norm3(
            hidden_states.float(), c_shift_msa, c_scale_msa
        ).type_as(hidden_states)
        
        # DEBUG: Log after cross-attention
        if os.environ.get("DEBUG_TRANSFORMER", "0") == "1" and hasattr(self, '_block_idx') and self._block_idx == 0:
            print(f"[FV Block {self._block_idx}] cross_attn_out: mean={hidden_states.float().mean().item():.6f}, std={hidden_states.float().std().item():.6f}")
        ...
        # DEBUG: Log after FFN
        if os.environ.get("DEBUG_TRANSFORMER", "0") == "1" and hasattr(self, '_block_idx') and self._block_idx == 0:
            print(f"[FV Block {self._block_idx}] ffn_out: mean={hidden_states.float().mean().item():.6f}, std={hidden_states.float().std().item():.6f}")
...
class WanActionTransformer3DModel(BaseDiT):
    ...
    def forward(
        ...
        **kwargs
        ...
        """
        Unified forward pass for both training and inference with KV caching.
        ...
        for block_idx, block in enumerate(self.blocks):
            block._block_idx = block_idx  # For debug logging
        ...
        # If cache-only mode, return early (matching original HY-WorldPlay behavior)
        if is_cache:
            return kv_cache
