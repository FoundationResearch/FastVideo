...
class BasicAVTransformerBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        # Keep dense paths (text/audio/cross-modal attention) on standard
        # kernels. VSA metadata is built from video latent tiles and is only
        # valid for video self-attention.
        dense_attn_backends = (
            AttentionBackendEnum.FLASH_ATTN,
            AttentionBackendEnum.TORCH_SDPA,
        )
    ...
    def forward(
        ...
        Args:
            ...
            skip_video_self_attn: Bool or per-sample tensor indicating STG
            perturbation for video self-attention. When tensor, shape should
            be [B] where 1/True means perturb (zero self-attn contribution)
            and 0/False means keep.
            skip_audio_self_attn: Bool or per-sample tensor indicating STG
            perturbation for audio self-attention. Same semantics as video.
        ...
        def _build_attn_keep_mask(
            ...
            # Only the final transformer block is perturbed for STG.
            bsz = values.shape[0]
        ...
        if run_vx:
            ...
            video_self_attn_mask = _build_attn_keep_mask(
                vx, skip_video_self_attn)
            if self.use_distributed_attention:
                vx = vx + self.attn1(norm_vx, pe=video.positional_embeddings, attention_mask=video_attention_mask) * vgate_msa * video_self_attn_mask
            else:
                vx = vx + self.attn1(norm_vx, pe=video.positional_embeddings) * vgate_msa * video_self_attn_mask
        ...
        if run_ax:
            ...
            audio_self_attn_mask = _build_attn_keep_mask(
                ax, skip_audio_self_attn)
            if self.use_distributed_attention:
                ax = ax + self.audio_attn1(norm_ax, pe=audio.positional_embeddings, attention_mask=audio_attention_mask) * agate_msa * audio_self_attn_mask
            else:
                ax = ax + self.audio_attn1(norm_ax, pe=audio.positional_embeddings) * agate_msa * audio_self_attn_mask
            # Text cross-attention: no SP mask needed (text is replicated, uses local attention)
...
class LTXModel(torch.nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            video_attention_mask: SP padding attention mask for video [B, padded_seq_len]
            audio_attention_mask: SP padding attention mask for audio [B, padded_seq_len]
            skip_video_self_attn: Bool or per-sample tensor [B] for STG
            perturbation of video self-attention.
            skip_audio_self_attn: Bool or per-sample tensor [B] for STG
            perturbation of audio self-attention.
            skip_cross_modal_attn: If True, skip A2V and V2A cross-modal
