...
def check_correctness(h, d, num_blocks, k,  num_iterations=20, error_mode='all'):
    results = {
        'gO': {'sum_diff': 0.0, 'sum_abs': 0.0, 'max_diff': 0.0, 'pt_max': 0.0, 'diff_max': 0.0, 'ratio_max': 0.0},
        'gQ': {'sum_diff': 0.0, 'sum_abs': 0.0, 'max_diff': 0.0, 'pt_max': 0.0, 'diff_max': 0.0, 'ratio_max': 0.0},
        'gK': {'sum_diff': 0.0, 'sum_abs': 0.0, 'max_diff': 0.0, 'pt_max': 0.0, 'diff_max': 0.0, 'ratio_max': 0.0},
        'gV': {'sum_diff': 0.0, 'sum_abs': 0.0, 'max_diff': 0.0, 'pt_max': 0.0, 'diff_max': 0.0, 'ratio_max': 0.0},
    }
    ...
    for _ in range(num_iterations):
        ...
        for name, (pt, bs) in zip(['gQ', 'gK', 'gV', 'gO'], [(pt_qg, bs_qg), (pt_kg, bs_kg), (pt_vg, bs_vg), (pt_o, bs_o)]):
            if bs is not None:
                ...
                abs_pt = torch.abs(pt)
                pt_max = torch.max(abs_pt).item()
                diff_max = torch.max(abs_diff).item()
                ratio = diff_max / (pt_max + 1e-12)

                results[name]['sum_diff'] += torch.sum(abs_diff).item()
                results[name]['sum_abs'] += torch.sum(abs_pt).item()
                rel_max_diff = torch.max(abs_diff) / torch.mean(abs_pt)
                results[name]['max_diff'] = max(results[name]['max_diff'], rel_max_diff.item())
                results[name]['pt_max'] = max(results[name]['pt_max'], pt_max)
                results[name]['diff_max'] = max(results[name]['diff_max'], diff_max)
                results[name]['ratio_max'] = max(results[name]['ratio_max'], ratio)
...
def generate_error_graphs(h, d, error_mode='all'):
    ...
    for config in test_configs:
        ...
        print(
            f"  Max | "
            f"gQ_diff {results['gQ']['diff_max']:<12.6e} gQ_gt {results['gQ']['pt_max']:<12.6e} gQ_ratio {results['gQ']['ratio_max']:<8.3f} | "
            f"gK_diff {results['gK']['diff_max']:<12.6e} gK_gt {results['gK']['pt_max']:<12.6e} gK_ratio {results['gK']['ratio_max']:<8.3f} | "
            f"gV_diff {results['gV']['diff_max']:<12.6e} gV_gt {results['gV']['pt_max']:<12.6e} gV_ratio {results['gV']['ratio_max']:<8.3f} | "
            f"gO_diff {results['gO']['diff_max']:<12.6e} gO_gt {results['gO']['pt_max']:<12.6e} gO_ratio {results['gO']['ratio_max']:<8.3f}"
        )
